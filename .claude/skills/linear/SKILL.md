...
New issues default to the **Triage** state. Set a resting state on creation (`--state "Backlog"` / `"Todo"`), or update it straight after with `linear issue update` — don't leave created issues in triage. Note: `create` does not accept `--priority`; set priority afterwards with `linear issue update ID --priority N`.
